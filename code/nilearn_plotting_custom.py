"""
CUSTOMIIZED Functions for surface visualization.
...
def plot_surf(surf_mesh, surf_map=None, bg_map=None,
              ...
              title=None, output_file=None, axes=None, figure=None, data_alpha=None, data_remove = None, **kwargs):
    ...
    if surf_map is not None:
        ...
        surf_map_alpha = load_surf_data(data_alpha)
        surf_map_remove = load_surf_data(data_remove)
        ...
        if avg_method == 'mean':
            ...
            surf_map_faces_alpha = np.mean(surf_map_alpha[faces], axis=1)
            surf_map_faces_remove = np.mean(surf_map_remove[faces], axis=1)
        elif avg_method == 'median':
            surf_map_faces = np.median(surf_map_data[faces], axis=1)
            surf_map_faces_alpha = np.median(surf_map_alpha[faces], axis=1)
            surf_map_faces_remove = np.median(surf_map_remove[faces], axis=1)
        ...
        print(np.unique(surf_map_faces))
        ...
        if bg_on_data:
            ...
            #face_colors[kept_indices] = cmap(surf_map_faces[kept_indices]) 
            face_colors[kept_indices] = ( cmap(surf_map_faces[kept_indices])* ( surf_map_faces_alpha[kept_indices,np.newaxis] )) \
                + ( face_colors[kept_indices] * (1-surf_map_faces_alpha[kept_indices,np.newaxis]) )
            
            removed_indices = np.where(surf_map_faces_remove == 1)[0]
            face_colors[removed_indices]=[0.1,0.1,0.1,0.9]
            
            #face_colors[kept_indices, 3] = surf_map_faces_alpha[kept_indices] * face_colors[kept_indices][:, 3] #THIS WORKS KINDA
